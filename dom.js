...
createDiv.insertAdjacentElement("beforebegin", header);
...
ulList.insertAdjacentHTML("beforebegin", elemnHTML);
...
const generateAutoCard = (brand, color, year) => {
  ...
  const totalyYear = curYear - year;
  return `
    <div class="autoCard">
        <h2>${brand.toUpperCase()} ${year}</h2>
        <p>Автомобиль ${brand.toUpperCase()} - ${year} года. Возраст авто - ${totalyYear} лет.</p>
        <p>Цвет: ${color} </p>
...
createDiv.insertAdjacentElement("beforebegin", carsDiv);
